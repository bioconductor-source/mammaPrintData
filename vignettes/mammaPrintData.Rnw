...
% \VignetteDepends{Biobase, limma, readxl}
...
Finally, the {\R} code chunk below was used to download and install
from \Bioc the packages required to prepared the present vignette.
if the are not available (e.g \Rpackage{readxl}).
...
<<get70genes, eval=TRUE, echo=TRUE, cache=FALSE>>=
